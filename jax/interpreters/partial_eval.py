...
def _remat_partial_eval(trace, _, f, tracers, params):
  ...
  if params['saveable_policy']:
    ...
    jaxpr1, jaxpr2, out_unknowns, in_used = partial_eval_jaxpr_custom(
    ...
    _, in_jaxpr_tracers = partition_list(in_used, in_jaxpr_tracers)
...
def partial_eval_jaxpr_custom(
  ...
  ) -> Tuple[Jaxpr, Jaxpr, List[bool], List[bool]]:
  jaxpr1, jaxpr2_, out_unknowns, out_inst, num_res = _partial_eval_jaxpr_custom(
      jaxpr, in_unknowns, saveable)
  _, used2 = partition_list(out_inst, out_unknowns)
  jaxpr2, in_used = dce_jaxpr(jaxpr2_, used2)
  assert len(out_unknowns) == len(jaxpr.outvars)
  return jaxpr1, jaxpr2, out_unknowns, in_used
...
def _partial_eval_jaxpr_custom(
  ...
  outs1, _ = partition_list(out_unknowns, jaxpr.outvars)
  jaxpr1_ = Jaxpr((), ins1, [*outs1, *residuals], eqns1)
  config.jax_enable_checks and core.check_jaxpr(jaxpr1_)
  keep_outs1 = [x.aval is not abstract_unit for x in jaxpr1_.outvars]
  jaxpr1, used1 = dce_jaxpr(jaxpr1_, keep_outs1)
  assert all(used1)
